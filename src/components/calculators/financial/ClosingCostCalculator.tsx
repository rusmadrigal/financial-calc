...
export function ClosingCostCalculator() {
  ...
  }, [loanAmount, originationPct, appraisalFee, titleInsurance, pointsPct, otherFees]);
  ...
  const handleExportPDF = () => {
    if (!hasResults) return;
    exportToPDF("Closing Cost Calculator", summaryData, tableHeaders, tableRows);
      ...
      <div className="grid gap-8 lg:grid-cols-3">
          ...
          <Card className="sticky top-24">
            ...
            <CardContent className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <Input type="number" min={0} step="0.1" value={originationPct} onChange={(e) => setOriginationPct(e.target.value)} />
              </div>
              <div className="space-y-2">
                <Label>Points (%)</Label>
                <Input type="number" min={0} step="0.25" value={pointsPct} onChange={(e) => setPointsPct(e.target.value)} />
              </div>
              <div className="space-y-2">
                <Label>Appraisal ($)</Label>
                <div className="relative">
                  <span className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground">$</span>
                  <Input type="number" min={0} value={appraisalFee} onChange={(e) => setAppraisalFee(e.target.value)} className="pl-7" />
                </div>
              </div>
              <div className="space-y-2">
                ...
                <div className="relative">
                  <span className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground">$</span>
                  <Input type="number" min={0} value={titleInsurance} onChange={(e) => setTitleInsurance(e.target.value)} className="pl-7" />
                </div>
              </div>
              <div className="space-y-2">
                ...
                <div className="relative">
                  <span className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground">$</span>
                  <Input type="number" min={0} value={otherFees} onChange={(e) => setOtherFees(e.target.value)} className="pl-7" />
                </div>
              </div>
        ...
        <div className="space-y-6 lg:col-span-2">
            ...
            <CardContent>
              <div className="grid gap-6 sm:grid-cols-2">
                <div>
                  ...
                  <p className="mt-1 text-2xl font-semibold">{usd.format(result.totalClosingCosts)}</p>
                </div>
                <div>
                  ...
                  <TableBody>
                    {result.items.map((item) => (
                      <TableRow key={item.label}>
                        ...
                        <TableCell className="text-right">{usd.format(item.amount)}</TableCell>
                      </TableRow>
                    ))}
                    <TableRow>
                      <TableCell className="font-semibold">Total</TableCell>
                      <TableCell className="text-right font-semibold">{usd.format(result.totalClosingCosts)}</TableCell>
                    </TableRow>
                  </TableBody>
                </Table>
              </CardContent>
          ...
          <Alert>
            ...
            <AlertDescription>
              Closing costs vary by lender and location. Add or adjust line items as needed for your scenario.
            </AlertDescription>
          </Alert>
        </div>
      </div>
