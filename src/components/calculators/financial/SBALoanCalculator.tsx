...
export function SBALoanCalculator() {
  const [principal, setPrincipal] = useState("150000");
  const [guaranteeFeePct, setGuaranteeFeePct] = useState("2");
  ...
  const result = useMemo(() => {
    return calculateAmortization({
      ...
      termYears: Math.max(0.5, parseFloat(termYears) || 10),
    });
  }, [principal, apr, termYears]);

  const guaranteeFee = useMemo(() => {
    const p = parseFloat(principal) || 0;
    const feePct = Math.max(0, Math.min(100, parseFloat(guaranteeFeePct) || 0));
    return (p * feePct) / 100;
  }, [principal, guaranteeFeePct]);
  ...
  const handleExportPDF = () => {
    if (!hasResults) return;
    exportToPDF("SBA Loan Calculator", summaryData, tableHeaders, tableRows);
    toast.success("PDF downloaded");
  };

  const handleExportExcel = () => {
    if (!hasResults) return;
    exportToExcel("SBA Loan Schedule", tableHeaders, tableRows);
    toast.success("Excel downloaded");
  };

  const handleDownloadFullSchedule = () => {
    if (!hasResults) return;
    exportToCSV("SBA-Loan-Schedule", tableHeaders, tableRows);
    toast.success("Schedule downloaded");
  };

  const handleReset = () => {
    setPrincipal("150000");
    setGuaranteeFeePct("2");
    setApr("10");
    setTermYears("10");
  ...
  return (
    <>
        ...
        <div className="lg:col-span-1">
          <Card className="sticky top-24">
            <CardHeader>
              ...
              <CardDescription>SBA loan terms</CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              ...
              <div className="space-y-2">
                <Label>Guarantee fee (%)</Label>
                <Input
                  type="number"
                  min={0}
                  max={100}
                  step="0.25"
                  value={guaranteeFeePct}
                  onChange={(e) => setGuaranteeFeePct(e.target.value)}
                />
              </div>
              <div className="space-y-2">
                <Label>APR (%)</Label>
        ...
        <div className="space-y-6 lg:col-span-2">
          <Card className="border-2">
            <CardHeader>
              ...
              <CardDescription>Payment and fee</CardDescription>
            </CardHeader>
            <CardContent>
              ...
              <CardContent>
                <div className="h-[300px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart
                      key={`line-${principal}-${guaranteeFeePct}-${apr}-${termYears}`}
                      data={chartDataLine}
                    >
                      <CartesianGrid
                        strokeDasharray="3 3"
            ...
            <Card>
              ...
              <CardContent>
                <div className="h-[300px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart
                      key={`bar-${principal}-${guaranteeFeePct}-${apr}-${termYears}`}
                      data={chartDataBar}
                    >
                      <CartesianGrid
                        strokeDasharray="3 3"
          ...
          <Alert>
            ...
            <AlertDescription>
              SBA guarantee fees vary by loan size and term. This is an estimate
              only.
            </AlertDescription>
          </Alert>
        </div>
      ...
      <Alert className="mt-8 border-2 border-destructive/50 bg-destructive/5">
        ...
        <AlertDescription>
          Not financial advice. Consult an SBA lender for actual terms.
        </AlertDescription>
      </Alert>
    </>
  );
